...
const Portfolio1 = () => {
    const initialUserState = { img: "", nam: "", about: "" };
    ...
    const submit = async (e) => {
        ...
        if (editId) {
            ...
            await axios.put(`http://localhost:7000/api/portfolio/update/${editId}`, user)
                .then((response) => {
            ...
            await axios.post("http://localhost:7000/api/portfolio/upload", user)
                .then((response) => {
    ...
    const deleteUser = async (userid) => {
        if (window.confirm("Are you sure you want to delete?")) {
            await axios.delete(`http://localhost:7000/api/portfolio/delete/${userid}`)
                .then((response) => {
    ...
    const startEdit = (selectedUser) => {
        ...
        setuser({ img: selectedUser.img, nam: selectedUser.nam , about: selectedUser.about}); // Form mein data bharo
    ...
    return (
        <section className="container mx-auto px-4 sm:px-8 w-3/4 lg:px-12 py-10 flex flex-col items-center gap-12 mt-20">
            <div className="container mx-auto">
                ...
                <div className="mx-auto max-w-4xl rounded-[2.5rem] border-2 border-[#A3FF00] bg-white p-8 shadow-xl md:p-12">
                    <h2 className="text-center mb-6 font-bold text-[#0B2C19] text-xl uppercase tracking-widest">
                        {editId ? "Edit Portfolio" : " New Portfolio"}
                    </h2>
                            ...
                            <div className="flex-1 space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        type="text"
                                        name="nam"
                                        value={user.nam} // Controlled input important hai
                                        onChange={handle}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        name="about"
                                        value={user.about} // Controlled input important hai
                                        rows={4}
                                        onChange={handle}
                            ...
                            <div className="flex flex-col md:flex-row gap-8">
                                <div className="flex flex-col items-center gap-3">
                                    <div className="w-32 h-32 rounded-xl border border-slate-200 overflow-hidden bg-slate-50">
                                        <img src={u.img} alt="Thumbnail" className="w-full h-full object-cover" />
                                    </div>
